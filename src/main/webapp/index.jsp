...
<%
    ...
    // 3) Читаем из запроса параметры фильтра
    ...
    // 4) Пагинация: посты и текущие/всего страницы
...
<div class="search-bar">
    <form action="<%= request.getContextPath() %>/posts" method="get" style="display:flex; flex-wrap:wrap; gap:8px;">
        <!-- Поисковая строка -->
        <input type="text" name="q" placeholder="Search..." value="<%= searchQuery %>" style="flex:1 1 200px;"/>
        <!-- Фильтр по категориям -->
        <select name="category">
            <option value="">All Categories</option>
            <% for (Category c : categories) { %>
            <option value="<%= c.getId() %>"
                    <%= (selectedCategory != null && c.getId() == selectedCategory) ? "selected" : "" %>>
                <%= c.getName() %>
            </option>
            <% } %>
        </select>
        ...
        <select name="tag">
            ...
            <option value="<%= t.getId() %>"
                    <%= (selectedTag != null && t.getId() == selectedTag) ? "selected" : "" %>>
                <%= t.getName() %>
            </option>
            <% } %>
        </select>
        ...
        <ul style="list-style:none; padding-left:0;">
            ...
            <li class="comment">
                ...
                <small class="comment-time">
                    By <span class="comment-author"><%= c.getUsername()!=null?c.getUsername():"Unknown" %></span>
                    at <%= c.getCreatedAt() %>
                </small>
            </li>
            <% } %>
...
<div class="pagination">
    <% for (int i = 1; i <= totalPages; i++) {
        ...
        if (!searchQuery.isEmpty())    url += "&q=" + URLEncoder.encode(searchQuery,"UTF-8");
        if (selectedCategory != null)   url += "&category=" + selectedCategory;
        if (selectedTag      != null)   url += "&tag=" + selectedTag;
        boolean isActive = (i == currentPage);
    %>
    <a href="<%= url %>" class="<%= isActive ? "active" : "" %>"
       style="<%= isActive ? "font-weight:bold;" : "" %>"><%= i %></a>
    <% } %>
</div>
<% } %>
